...
function addCrewMember(member: string, spaceship: {
  name: string;
  crewLimit: number;
  crew: string[];
  inMission: boolean;
  ...
  if (spaceship.crew.length >= spaceship.crewLimit) {
    alert(`Não é possível adicionar ${member}. Limite de tripulantes atingido.`);
  } else {
...
function firstMenuOption() {
  ...
  const crewLimit = Number.parseInt(prompt(`Quantos tripulantes a ${name} suporta?`) || '0');
...
while (userOption !== 5) {
  ...
  userOption = Number.parseInt(prompt(menu) || '0');
